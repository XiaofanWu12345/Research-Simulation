...
PLIST = ["MaxWt", "PriE", "Algo_2", "Algo_3"]
...
def get_S_qavg_lists(Vs, policy, S):
    ...
    for t in range(1, ITER + 1):
        ...
        qlist = ht.updateQ(qlist[0], qlist[1], qlist[2], lambdaRv_list[0], lambdaRv_list[1], lambdaRv_list[2]) # Update arrivals on queue lengths in qlist
        if policy == "MaxWt":
