...
def main(args):
    for Vs in VLIST:
        for policy in PLIST:
            ...
            # set of V values
            # if PLIST.index(policy) == 0:
            #     headers = ', '.join([str(e) for e in S_headers])
            #     print ' , ,' + headers
            # qavgs = ', '.join([str(e) for e in qavg_values])
            # print policy + ', Avg Queue Length, ' + qavgs
            # etqs = ', '.join([str(e) for e in etq_values])
            # print policy + ', (1 - S) * qavg, ' + etqs
