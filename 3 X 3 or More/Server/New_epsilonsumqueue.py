...
VLIST = ML.VLIST
#a list of lists whose elements are the values of v
...
def main(args):
    for Vs in VLIST:
        f = VLIST.index(Vs)
        plt.figure(f + 1)
        subfigure = read.readData(str(VLIST.index(Vs)) + 'th Vs' + '.csv')[1]
        ...
        for i in range(len(PLIST)):
            ...
            plt.plot(subfigure[0], subfigure[i+1],
                    ...
                    linestyle = stylelist[i], marker = markerlist[i], markersize = 12 )
